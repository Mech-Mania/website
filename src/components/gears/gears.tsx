...
function Gears(props: any) {
    ...
    const currWidth =  ((window.innerWidth<1024 ? 12 : 0) + props.width)
    ...
    return (
        <>
            {teeth}
            <div style={{ width: currWidth + "rem", height: props.height + "rem" }} className="bg-black box-content z-10 rounded-[2rem]">
                {props.children}
            </div>
        </>
    );
